    ...
    IERC2981
{
    ...
    constructor(address initialOwner) ERC1155("") Ownable(initialOwner) {
        ...
        mintPrice[0] = 10 ether; // Initial mint price (in S)
        incrementAmount[0] = 2 ether; // Increment price after each mint (in S)
    }
    ...
    function uri(
        ...
        if (!tokenUnlocked[_id]) {
            revert TokenLocked(_id, tokenUnlocked[_id]);
        }
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view override(IERC165, ERC1155) returns (bool) {
        return
